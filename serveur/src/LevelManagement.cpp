...
void LevelManager::update(float gameTime) {
    ...
    if (levelTime >= levels[currentLevel].duration &&
        getRemainingEnemies() == 0) {
        levelComplete = true;
    }
}
...
bool LevelManager::isLevelComplete() const { return levelComplete; }
...
size_t LevelManager::getCurrentLevel() const { return currentLevel + 1; }
