    ...
    namespace systems {
        ...
        void GameLogicSystem::checkCollisions() {
            ...
            for (size_t i = 0; i < positions.size(); ++i) {
                ...
                for (size_t j = i + 1; j < positions.size(); ++j) {
                    if (!positions[j].has_value()) continue;
        ...
        void GameLogicSystem::handleEnemyDeath(Entity enemy) {
            ...
            if (enemyData.has_value()) {
                scoreManager.updateScore(enemyData->lastDamageSource, enemyData->pointValue);
