...
void createPlayer(Child &child, const std::string &name, const std::string &color, const std::string &uuid)
{
...
void disconnect(ECS &ecs, const ReceiveEvent &rec_event, SparseArray<Room> &rooms,
                SparseArray<Tag> &tags,
                SparseArray<Child> &children)
{
    int nbPlayer = (rec_event.payload.contains("nbp")) ? rec_event.payload["nbp"].get<int>() : 1;
    ...
    for (size_t i = 0; i < rooms.size() && i < tags.size() && i < children.size(); ++i) {
        ...
        if (!ro || !tag || ro.value()._clients_uuid.find(rec_event.sender_uuid) == ro.value()._clients_uuid.end()) 
            continue;
        ro.value()._clients_uuid.erase(ro.value()._clients_uuid.find(rec_event.sender_uuid));
        ...
        if (ro.value()._nbPlayer <= 0)
            ...
            if (ro.value()._master == rec_event.sender_uuid) {
                ...
                ecs.post<RequestEvent>({JOIN_ROOM, {{"master", true}, {"id", tag.value()._id}, {"name", ro.value()._name}, {"slot", ro.value()._nbPlayer}}, ro.value()._master});
            }
        }
    }
              ...
              SparseArray<Child> &children)
{
    ...
    for (size_t i = 0; i < rooms.size(); ++i) {
        ...
        if (ro && ro.value()._name == rec_event.payload["room_name"].get<std::string>()) {
            roomE = Entity(i);
            break;
        }
    }
    if (roomE == -1) {
        ...
        if ((rooms[roomE].value()._nbPlayer += nbPlayer) > 8)
            return;
        rooms[roomE].value()._clients_uuid.insert({rec_event.sender_uuid, WAITING});
        rooms[roomE].value()._nbPlayer += nbPlayer;
    }
    auto namesP = rec_event.payload["n_player"].get<std::vector<std::string>>();
    auto colorsP = rec_event.payload["c_player"].get<std::vector<std::string>>();
    for (int i = 0; i < nbPlayer; i++)
        createPlayer(children[roomE].value(), namesP[i], colorsP[i], rec_event.sender_uuid);
    for (const auto &[uuid, _] : rooms[roomE].value()._clients_uuid) {
        if (uuid == rec_event.sender_uuid)
            ecs.post<RequestEvent>({JOIN_ROOM, {{"master", true}, {"id", tags[roomE].value()._id}, {"name", rooms[roomE].value()._name}, {"slot", rooms[roomE].value()._nbPlayer}}, uuid});
        if (uuid == rec_event.sender_uuid)
            ecs.post<RequestEvent>({JOIN_ROOM, {{"id", tags[roomE].value()._id}, {"name", rooms[roomE].value()._name}, {"slot", rooms[roomE].value()._nbPlayer}}, uuid});
    }
    return;
}

void launchGame(ECS &ecs, const ReceiveEvent &rec_event, SparseArray<Room> &rooms, SparseArray<Tag> &tags)
{
                                       ...
                                       SparseArray<Child> &children)
{
    ...
    for (size_t i = 0; i < rooms.size() && i < children.size(); ++i) {
        ...
        if (ro && child && ro.value()._clients_uuid.find(rec_event.sender_uuid) != ro.value()._clients_uuid.end()) {
