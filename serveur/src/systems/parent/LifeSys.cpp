        ...
        if (ro && st && child && ro.value()._state == IN_GAME) {
            for (int j = 0; j < st.value()._enemies.size(); j++) {
                ...
                if (ennemy.spawn_tic <= 0) {
                    ...
                    //         rtype::server::EnemyAI::BehaviorType toSpawn =
                    //         spawnQueue.pop(); std::vector<RequestEvent> resp;
                    //         switch (toSpawn) {
                    //         case rtype::server::EnemyAI::BehaviorType::BOSS:
                    //             resp = createBoss(child.value()._ecs_child,
                    //             enemy); break;
                    //         case
                    //         rtype::server::EnemyAI::BehaviorType::SINUSOIDAL:
                    //             resp =
                    //             createSineEnemy(child.value()._ecs_child,
                    //             enemy); break;
                    //         case rtype::server::EnemyAI::BehaviorType::CHASE:
                    //             resp = createChase(child.value()._ecs_child,
                    //             enemy); break;
                    //         case
                    //         rtype::server::EnemyAI::BehaviorType::UPDOWN:
                    //             resp = createUpDown(child.value()._ecs_child,
                    //             enemy); break;
                    //         default:
                    //             break;
                    //         }
                    ...
                    // ennemy); auto resp =
                    // createCircle(child.value()._ecs_child, ennemy); auto resp
                    // = createUpDown(child.value()._ecs_child, ennemy); auto
                    ...
                    //     for (const auto &[uuid, _] :
                    //     ro.value()._clients_uuid)
                    //         ecs.post<RequestEvent>(
                    //             {SV_CREATE_ENTITY, evt.payload, uuid});
                    // }
                    // st.value()._enemies.erase(st.value()._enemies.begin() +
                    // j); j++;
