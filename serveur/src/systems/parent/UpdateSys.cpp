                           ...
                           SparseArray<Child> &children)
{
    for (size_t i = 0; i < rooms.size() && i < children.size(); ++i) {
        ...
        if (ro && child && ro.value()._state == IN_GAME) {
            ...
            const auto &players = child.value()._ecs_child.get_components<PlayerData>();
            const auto &positions = child.value()._ecs_child.get_components<Position>();
            const auto &velocities = child.value()._ecs_child.get_components<Velocity>();
            const auto &hitboxes = child.value()._ecs_child.get_components<HitBox>();
            const auto &healths = child.value()._ecs_child.get_components<Health>();
            const auto &scores = child.value()._ecs_child.get_components<Score>();

            ro.value()._lastUpdate = ((ro.value()._lastUpdate + 1) > 10) ? 1 : (ro.value()._lastUpdate + 1);
