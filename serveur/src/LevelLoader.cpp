...
void LevelLoader::reset(void) {
    ...
    try {
        if (std::filesystem::exists(dossier) && std::filesystem::is_directory(dossier)) {
            for (const auto& entry : std::filesystem::directory_iterator(dossier)) {
                cmpt += 1;
                _map.insert({cmpt, "level/" + entry.path().filename().string()});
                std::cout << entry.path().filename().string() << std::endl;
            }
        } else {
            std::cerr << "Le chemin spécifié n'est pas un dossier ou n'existe pas." << std::endl;
        }
