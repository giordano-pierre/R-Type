...
template <typename Component> class SparseArray {
  public:
    ...
    ~SparseArray() = default;
    auto operator=(const SparseArray &other)
        -> SparseArray &; // copy assignment operator
    auto operator=(SparseArray &&other) noexcept
        -> SparseArray &; // move assignment operator
    ...
    auto emplace_at(size_type pos, Params &&...args)
        -> reference_type; // build the component object in-place.
