...
public:
  Gui(const std::string &framesPath, int frameCount, int frameDurationMs)
      : m_frameCount(frameCount),
        m_frameDuration(sf::milliseconds(frameDurationMs)), m_currentFrame(0) {
    for (int i = 1; i < frameCount; ++i) {
      sf::Texture texture;
      std::string filename =
          framesPath + "/frame-" + std::to_string(i) + ".png";
      if (!texture.loadFromFile(filename)) {
        std::cerr << "Erreur de chargement de l'image: " << filename
                  << std::endl;
        continue;
      }
  ...
  void run() {
    sf::RenderWindow window(sf::VideoMode::getDesktopMode(), "Lunar Pirate GUI",
                            sf::Style::Fullscreen);
    ...
    while (window.isOpen()) {
      ...
      m_sprite.setScale(static_cast<float>(windowSize.x) / textureSize.x,
