...
void WindowSys::drawText(SparseArray<Position> &positions,
  ...
  for (size_t i = 0;
    ...
    if (pos && box && tex) {
      ...
      TupleFloat sizeGoal = {tex.value()._sizeInBox.x * box.value()._client.x,
                             tex.value()._sizeInBox.y * box.value()._client.y};
      TupleFloat boxOrigine = {
          pos.value()._client.x - (box.value()._client.x / 2),
          pos.value()._client.y - (box.value()._client.y / 2)};
      TupleFloat posInBox = {
          (box.value()._client.x - sizeGoal.x) * tex.value()._posInBox.x,
          (box.value()._client.y - sizeGoal.y) * tex.value()._posInBox.y};
      auto currentSize = tex.value()._text.getLocalBounds();
      auto coef = std::min(sizeGoal.x / currentSize.width,
                           sizeGoal.y / currentSize.height);
      tex.value()._text.setScale({coef, coef});
      tex.value()._text.setPosition(
          {boxOrigine.x + posInBox.x + (sizeGoal.x / 2),
           boxOrigine.y + posInBox.y + (sizeGoal.y / 2)});
...
void WindowSys::operator()(ECS &ecs, const FrameEvent &,
  ...
  TupleUInt serverSize = (windows.size() > 0 && windows[0])
                             ...
                             : TupleUInt{1920, 1080};
  auto inputConfig = (windows.size() > 0 && windows[0])
