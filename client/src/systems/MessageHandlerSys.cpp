...
void createDrawable(ECS &ecs, Entity &entity, SFMLObjects &SFMLObj,
    ...
    void createEntity(ECS & ecs, Entity & entity, const ReceiveEvent &rec_event,
        ...
        if (rec_event.payload.contains("type") &&
            ...
            ecs.add_component<Tag>(
                entity, {rec_event.payload["type"].get<EntityType>(),
        ...
        if (rec_event.payload.contains("vel"))
            ecs.add_component<Velocity>(
                entity, {rec_event.payload["vel"]["x"].get<int>(),
        ...
        if (rec_event.payload.contains("hp"))
            ecs.add_component<Health>(entity,
                                      {rec_event.payload["hp"].get<int>()});
        if (rec_event.payload.contains("sc"))
            ecs.add_component<Score>(entity,
                                     {rec_event.payload["sc"].get<int>()});
        if (rec_event.payload.contains("pu")) {
            // std::cout<<"^HERE SDFJISQJDIJQSIDJISQJDIJSJQDIJQSI->
            // "<<rec_event.payload["pu"].get<int>()<<std::endl;

            ecs.add_component<Powerup>(
                entity, Powerup(rec_event.payload["pu"].get<int>()));
    ...
    bool entityExist(const ReceiveEvent &rec_event,
    ...
    void MessageHandlerSys::operator()(
        ...
        case SV_CREATE_PLAYER: {
            ...
            ecs.post<RequestEvent>(
                {SV_CREATE_PLAYER, {{"idr", myRoom._idRoom}}});
            break;
        }
        case SV_GAME_OVER: {
        ...
        case SV_UPDATE_ENTITY: {
            if (!entityExist(rec_event, tags)) {
                ...
                Entity entity = getEntityByID(
                    rec_event.payload["id"].get<std::string>(), tags);
