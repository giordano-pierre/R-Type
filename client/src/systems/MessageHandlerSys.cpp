...
void joinRoom(ECS &ecs, const ReceiveEvent &rec_event,
              Window &myWindow)
{
...
void createDrawable(ECS &ecs, Entity &entity, Window &myWindow, EntityType type)
{
    switch (type) {
        case PLAYER:
        ...
        case SHOOT1:
            ecs.add_component<Drawable>(entity,  {
                myWindow._myTextures.getTexture("assets/images/shot/purple_shot.png"),
                {251, 144},
                {251, 144},
                1,
                2
...
void createEntity(ECS &ecs, Entity &entity, const ReceiveEvent &rec_event, Window &myWindow)
{
    if (rec_event.payload.contains("type") && rec_event.payload.contains("id")) {
        ecs.add_component<Tag>(entity, {rec_event.payload["type"].get<EntityType>(), rec_event.payload["id"].get<std::string>()});
        createDrawable(ecs, entity, myWindow, rec_event.payload["type"].get<EntityType>());
    }
    if (rec_event.payload.contains("hit"))
        ecs.add_component<Hitbox>(entity, {{rec_event.payload["hit"]["x"].get<float>(), rec_event.payload["hit"]["y"].get<float>()}});
    if (rec_event.payload.contains("pos"))
        ecs.add_component<Position>(entity, {rec_event.payload["pos"]["x"].get<float>(), rec_event.payload["pos"]["y"].get<float>()});
    if (rec_event.payload.contains("vel"))
        ecs.add_component<Velocity>(entity, {rec_event.payload["vel"]["x"].get<int>(), rec_event.payload["vel"]["y"].get<int>()});
    if (rec_event.payload.contains("name") && rec_event.payload.contains("color"))
        ecs.add_component<PlayerData>(entity, {rec_event.payload["name"].get<std::string>(), rec_event.payload["color"].get<std::string>()});
...
bool entityExist(const ReceiveEvent &rec_event, const SparseArray<Tag> &tags)
{
    for (size_t i = 0; i < tags.size(); ++i) {
        ...
        if (tag && tag.value()._id == rec_event.payload["id"].get<std::string>())
            return true;
    }
    return false;
}

void createPlayer(ECS &ecs, const ReceiveEvent &rec_event, Window &myWindow, const SparseArray<Tag> &tags)
{
...
{
    if (rec_event.payload.contains("pos") && entity < positions.size() && positions[entity]) {
        positions[entity].value()._server = {rec_event.payload["pos"]["x"].get<float>(), rec_event.payload["pos"]["y"].get<float>()};
        positions[entity].value()._needUpdate = true;
    }
    if (rec_event.payload.contains("vel") && entity < velocities.size() && velocities[entity]) {
        velocities[entity].value()._current = {rec_event.payload["vel"]["x"].get<int>(), rec_event.payload["vel"]["y"].get<int>()};
    }
    if (rec_event.payload.contains("hp") && entity < healths.size() && healths[entity]) {
        healths[entity].value()._health = rec_event.payload["hp"].get<int>();
    }
    if (rec_event.payload.contains("sc") && entity < scores.size() && scores[entity]) {
        scores[entity].value()._score = rec_event.payload["sc"].get<int>();
    }
    if (rec_event.payload.contains("lu") && entity < lastups.size() && lastups[entity]) {
        lastups[entity].value()._lastUpdate = rec_event.payload["lu"].get<int>();
    }
}

void killEntity(ECS &ecs, const ReceiveEvent &rec_event, const SparseArray<Tag> &tags)
{
    for (size_t i = 0; i < tags.size(); ++i) {
        const auto &tag = tags[i];
...
void MessageHandlerSys::operator()(ECS &ecs, const ReceiveEvent &rec_event,
                                   ...
                                   SparseArray<LastUpdate> &lastups)
{
    ...
    case SV_UPDATE_ENTITY: {
        if (!entityExist(rec_event, tags)) {
            ...
            Entity entity = getEntityByID(rec_event.payload["id"].get<std::string>(), tags);
            updateEntity(entity, rec_event, positions, velocities, healths, scores, lastups);
        }
        return;
    }
    case SV_KILL_ENTITY: {
